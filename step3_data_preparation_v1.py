...
df_features = df.drop(columns=DROP_COLS + ["patient_nbr"])
...
# [EDA] Medication columns: levels are No / Steady / Up / Down.
# [ASSUMPTION] Treated as ordinal: No < Steady < changed (Up or Down).
#              Up/Down direction collapsed to a single "dose-changed" state.
MEDICATION_COLS = [
...
all_features = set(df_features.columns) - {"readmit_30"}
...
# ── Medication ordinal ────────────────────────────────────────────────────────
# [ASSUMPTION] No < Steady < Up/Down (dose changed). OrdinalEncoder with
#              consistent category list across all medication columns.
MED_CATEGORIES = [["No", "Steady", "Up", "Down"]] * len(MEDICATION_COLS)
...
WORKING ASSUMPTIONS — VALIDATE AT MODELLING STAGE:
  [A-1]  Medication ordinal: No < Steady < Up/Down; Up/Down direction collapsed
